...
// In the browser, prefer same-origin relative API calls (avoids port mismatch).
// On the server (SSR/actions), fall back to NEXT_PUBLIC_APP_URL.
const API_BASE_URL =
...
class ApiClient {
  ...
  // AI Response Generation (optional model: e.g. 'gemini-2.0-flash' to test a specific model)
  async generateResponse(message: string, agentPrompt?: string, model?: string, messages?: { role: string, content: string }[]): Promise<ApiResponse<{ text: string }>> {
    return this.request<{ text: string }>('/generate-response', {
      method: 'POST',
      body: JSON.stringify({ message, agentPrompt, ...(model ? { model } : {}), ...(messages ? { messages } : {}) }),
    })
  }
