...
export function AppProvider({ children }: AppProviderProps) {
  ...
  const createAgent = useCallback(async (agentData: Omit<Agent, 'id' | 'createdAt' | 'updatedAt'>): Promise<boolean> => {
    try {
      // Try to create agent in the backend first
      const response = await api.createAgent(agentData)

      let newAgent: Agent
      if (response.success && response.data) {
        // Handle nested data wrappers from the Next.js API proxy returning the FastAPI object
        ...
        // Fallback: create locally if backend is unreachable
        console.warn('Backend unavailable, saving agent locally:', response.error)
        newAgent = {
          ...defaultCompanionConfig,
          ...agentData,
